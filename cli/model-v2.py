...
class models(object):
	...
	def register_app(self, params):
		...
		if appHost == None:
			...
			if params['app_domain'] in appCheck['app_domain']:
				return "Domain Exist!"				
			else:	
				if appCheck != None:
					dbMongo.antares_app.update({'app_name':appCheck['app_name']},{'$push':{'app_domain':params['app_domain']}})
				else:
...
if __name__ == '__main__':
	...
	print model.register_app({
		'app_name':'Antares',
		'app_domain':'http://antares-v4.inzpire.co.id',
